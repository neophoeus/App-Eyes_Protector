...
class TestI18n(unittest.TestCase):
    ...
    def test_translation_retrieval(self):
        set_language("en")
        self.assertEqual(t("countdown"), "Every 20 minutes, look 20 feet (about 6m) away for 20 seconds")
        self.assertEqual(t("complete"), "Break complete, ease back to work.")
        self.assertEqual(t("warning_text", remaining=5), "Break starting soon (5s remaining) | Click the floating widget to pause")

        set_language("zh-TW")
        self.assertEqual(t("countdown"), "每20分鐘，花20秒鐘，看20英尺 (約6公尺) 外")
        self.assertEqual(t("warning_text", remaining=10), "休息即將開始 (剩餘 10 秒)  |  可點選右下角懸浮球暫停")

        set_language("zh-CN")
        self.assertEqual(t("countdown"), "每20分钟，花20秒钟，看20英尺 (约6米) 外")
    ...
    @mock.patch("ctypes.windll.kernel32.GetUserDefaultUILanguage")
    def test_get_system_language_ctypes_zh_hk(self, mock_get_lang):
        mock_get_lang.return_value = 0x0c04  # zh-HK (Hong Kong) -> zh-TW
        import eyes_protector.i18n
        eyes_protector.i18n._lang = None
        self.assertEqual(get_system_language(), "zh-TW")

    @mock.patch("ctypes.windll.kernel32.GetUserDefaultUILanguage")
    def test_get_system_language_ctypes_zh_sg(self, mock_get_lang):
        mock_get_lang.return_value = 0x1004  # zh-SG (Singapore) -> zh-CN
        import eyes_protector.i18n
        eyes_protector.i18n._lang = None
        self.assertEqual(get_system_language(), "zh-CN")

    @mock.patch("ctypes.windll.kernel32.GetUserDefaultUILanguage")
    def test_get_system_language_ctypes_zh_mo(self, mock_get_lang):
        mock_get_lang.return_value = 0x1404  # zh-MO (Macau) -> zh-TW
        import eyes_protector.i18n
        eyes_protector.i18n._lang = None
        self.assertEqual(get_system_language(), "zh-TW")

    @mock.patch("ctypes.windll.kernel32.GetUserDefaultUILanguage")
    def test_get_system_language_ctypes_zh_my(self, mock_get_lang):
        mock_get_lang.return_value = 0x1804  # zh-MY (Malaysia) -> zh-CN
        import eyes_protector.i18n
        eyes_protector.i18n._lang = None
        self.assertEqual(get_system_language(), "zh-CN")
